...
{
    public class StaffPlan_DAL : BaseDal<ADOProvider>
    {
        public List<StaffPlan> StaffPlan_GetList(int pageNumber, int pageSize, string filter,int LanguageCode, out int total, out ToTalMonth ListTotalMonth)
        {
            try
            {
                ...
                var list = UnitOfWork.Procedure<StaffPlan>("StaffPlan_GetList", param).ToList();
                var userId = Global.CurrentUser.UserID;
                param = HttpRuntime.Cache.Get("StaffPlan_GetList-" + ERP.Framework.Common.Utils.GetKeyFromParam(param as object) + "-" + userId + "-output") as DynamicParameters;
        ...
        public SystemMessage StaffPlan_Save(StaffPlan data)
        {
            ...
            try
            {
                ...
                UnitOfWork.ProcedureExecute("StaffPlan_Save", param);
        ...
        public SystemMessage StaffPlan_Delete(int AutoID)
        {
            SystemMessage systemMessage = new SystemMessage();
            try
            {
                ...
                UnitOfWork.ProcedureExecute("StaffPlan_Delete", param);
                systemMessage.IsSuccess = true;
                return systemMessage;
            }
            catch (Exception e)
        ...
        public List<Employee> Staff_GetALL()
        {
            try
            {
                //UnitOfWork.ConnectionString = ERP.Framework.Common.Utils.GetSetting<string>("NovaonADConnection");
                var list = UnitOfWork.Procedure<Employee>("Staff_GetALL", useCache: true).ToList();
                return list;
            }
            catch (Exception ex)
            {
