...
public class NovaonADDBM : IDisposable
{
    ...
    private string strConnect = Config.GetConfigByKey("NovaonADConnection");
    static public string AppSettingsDBKey = "NovaonADConnection";
    ...
    /// <summary>
    /// Constructor: tạo SqlConnection cho object NovaonADDBM với ConnectionString được truyền vào
    /// </summary>
    public NovaonADDBM()
    {
    ...
    /// <summary>
    /// Constructor: gán SqlConnection cho object NovaonADDBM với SqlConnection được truyền vào
    /// </summary>
    /// <param name="SqlConn"></param>
    public NovaonADDBM(SqlConnection SqlConn)
    {
    ...
    // Example: BSS.NovaonADDBM.ExecStore("sp_test", new { var1, var2, var3 });

    /// <summary>
    /// Hàm tĩnh, cho phép execute store storeName trong DB mà không cần khởi tạo object NovaonADDBM, dùng default ConnectionString (được set trong key AppSettingsDBKey = "MSSQL" ở file app.config hoặc web.config)
    /// </summary>
    ...
    /// <summary>
    /// Hàm tĩnh, cho phép execute store storeName trong DB mà không cần khởi tạo object NovaonADDBM, với connection string strConnect được truyền vào
    /// </summary>
    ...
    /// <summary>
    /// Hàm tĩnh, cho phép execute store storeName trong DB, với tham số parameters mà không cần khởi tạo object NovaonADDBM, dùng default ConnectionString (được set trong key AppSettingsDBKey = "MSSQL" ở file app.config hoặc web.config)
    /// </summary>
    /// <param name="storeName"></param>
    /// <param name="parameters"></param>
    static public string ExecStore<T>(string storeName, object parameters=null) where T : class
    {
        List<object> outValues = null;
        return ExecStore(ConnectionString, storeName, parameters, QueryTypeNoOutput, null, out outValues);
    }
    /// <summary>
    /// Hàm tĩnh, cho phép execute store storeName trong DB, với tham số parameters mà không cần khởi tạo object NovaonADDBM, với connection string strConnect được truyền vào
    /// </summary>
    /// <param name="strConnect"></param>
    /// <param name="storeName"></param>
    /// <param name="parameters"></param>
    ...
    // Example: BSS.NovaonADDBM.ExecStore("sp_test", new { var1, var2, var3 }, "Name", out sName);
    ...
    static private string ExecStore(SqlConnection sqlConn, SqlTransaction sqlTransac, string storeName, Hashtable htParams, int queryType, bool isAsync, HandleCallbackFunc HandleCallback, List<string> outVarNames, out List<object> outValues)
    {
        outValues = null;
    ...
    #region Các hàm động, cho phép execute store với SqlConn, SqlTransac, storeName, htParam đã được set với object NovaonADDBM. Dùng khi cần có transaction với các câu query khác
    // Ví dụ: BSS.NovaonADDBM NovaonADDBM = new BSS.NovaonADDBM();
    //        NovaonADDBM.BeginTransac()
    //        NovaonADDBM.SetStoreNameAndParams("sp_test", new { var1, var2, var3 });
    //        NovaonADDBM.ExecStore();
    //        NovaonADDBM.CommitTransac();
    ...
    /// <para>BSS.NovaonADDBM NovaonADDBM = new BSS.NovaonADDBM();</para>
    /// <para>string err = NovaonADDBM.SetStoreNameAndParams("dbo.sp_LogActivity_Insert", new { LogContent = "Enter log", UserID = 20, ObjectGUID = Guid.NewGuid(), IP = "10.3.4.43" }).ExecStore();</para>
    /// </summary>
    ...
    /// <summary>
    /// Hàm thực thi các store sau khi chạy lệnh SetStoreNameAndParams để điền các tham số đầu vào và tên store cần thực thi và có giá trị trả ra
    /// BSS.NovaonADDBM NovaonADDBM = new BSS.NovaonADDBM();
    /// DataTable dt = null;
    ...
    /// </summary>
    /// <returns></returns>
    public NovaonADDBM ClearParams()
    {
        htParams.Clear();
        return this;
    }
    ...
    {
        if (parameters == null) return "Error: parameters is null @NovaonADDBM.AddParams";
    ...
    public NovaonADDBM SetQueryString(string queryString)
    {
    ...
    {
        if (_SqlConn == null) return "Error: SqlConn is null @NovaonADDBM.ExecQueryString";
        if (string.IsNullOrEmpty(_QueryString)) return "Error: QueryString is null @NovaonADDBM.ExecQueryString";
        string msg = "";
    ...
    /// DataTable dt = null;
    /// string err = NovaonADDBM.GetOne("dbo.sp_LogActivity_SelectOne", new { ID = 5 }, out objLogActivity);
    /// </summary>
    ...
    /// DataTable dt = null;
    /// string err = NovaonADDBM.GetList("dbo.sp_LogActivity_SelectOne", new { ID = 5 }, out objListLogActivity); 
    /// </summary>
    /// <typeparam name="T">Kiểu đối tượng chứa biến</typeparam>
    /// <typeparam name="TL">Kiểu đối tượng cần lấy</typeparam>
    /// <param name="storeName">Tên store</param>
    ...
    static private string GetOne<T>(NovaonADDBM NovaonADDBM, string storeName, Hashtable htParams, out T oneItem)
    {
        ...
        if (list == null) return "Error: list is null @NovaonADDBM.GetOne";
    ...
    static private string GetList<T>(NovaonADDBM NovaonADDBM, string storeName, Hashtable htParams, out List<T> list)
    {
        ...
        DataTable dt = null;
        if (NovaonADDBM == null) msg = ExecStore(ConnectionString, storeName, htParams, out dt);
        else msg = NovaonADDBM.ExecStore(out dt);
