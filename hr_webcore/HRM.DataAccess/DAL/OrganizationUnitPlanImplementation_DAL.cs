...
{
    public class OrganizationUnitPlanImplementation_DAL : BaseDal<ADOProvider>
    {
        public List<OrganizationUnitPlan> OrganizationUnitPlanImplementation_GetList(int pageNumber, int pageSize, string filter,int LanguageCode, out int total, out ToTalMonth ListTotalMonth)
        {
            try
            {
                ...
                var list = UnitOfWork.Procedure<OrganizationUnitPlan>("OrganizationUnitPlanImplementation_GetList", param).ToList();
                var userId = Global.CurrentUser.UserID;
                param = HttpRuntime.Cache.Get("OrganizationUnitPlanImplementation_GetList-" + ERP.Framework.Common.Utils.GetKeyFromParam(param as object) + "-" + userId + "-output") as DynamicParameters;
        ...
        public SystemMessage OrganizationUnitPlanImplementation_Delete(int AutoID)
        {
            SystemMessage systemMessage = new SystemMessage();
            try
            {
                ...
                UnitOfWork.ProcedureExecute("OrganizationUnitPlanImplementation_Delete", param);
                systemMessage.IsSuccess = true;
                return systemMessage;
            }
            catch (Exception e)
        ...
        public List<OrganizationUnitPlan> ImPortOrganizationUnitPlanImplementation(List<OrganizationUnitPlan> data, out bool IsSuccess)
        {
            try
            {
                var param = new DynamicParameters();
                param.Add("@LanguageID", HRM.Common.Global.CurrentLanguage);
                param.Add("@OrganizationUnitPlanImplementation", data.ToUserDefinedDataTable(), DbType.Object);
                var list = UnitOfWork.Procedure<OrganizationUnitPlan>("ImPortOrganizationUnitPlanImplementation", param,useCache:false).ToList();
                IsSuccess = true;
                return list;
            }
            catch (Exception ex)
            {
                IsSuccess = false;
                return new List<OrganizationUnitPlan>();
                throw;
            }
