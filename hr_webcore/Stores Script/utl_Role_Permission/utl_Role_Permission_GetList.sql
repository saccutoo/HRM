...
GO
CREATE PROCEDURE [dbo].[utl_Role_Permission_GetList]
...
IF(@type = 1)
	...
	set @cSqlFrom =   N'SELECT a.*,GridName=(select iif('+CAST(@Language as nvarchar)+'=5,Note,TableName) from Sys_Table where id=a.GridId )
				,PermissionName=(select iif('+CAST(@Language as nvarchar)+'=5,Name,NameEN) from Sec_Role  where a.PermissionId=RoleID)
				FROM ERP_v2_20190327.dbo.utl_Role_Permission a'
