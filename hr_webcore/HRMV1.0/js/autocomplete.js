(function ($) {
    ...
    $.fn.autocomplete = function (options, callback, idSearch, idSelected, txtSelected, numberCol, showAdd) {
        var tblWidth = options.width;
        if (showAdd == undefined || showAdd == null) {
            showAdd = true;
        }
        // default parameters
        var settings = $.extend({
        ...
        var el = {
            ...
            ddDivContent: $("<div>"),//, { style: "overflow: auto;height: 300px;background-color: #FFF;" }),
            ddTable: $("<table></table>", { style: "width:" + settings.width }),
            ddTableCaption: $("<caption>" + settings.norecord + "</caption>"),
            ddDivButon: $("<div>", { style: "background-color:#FFF;position: relative;" }),
            ddTextbox: $("<input type='text' id=" + idSearch + " class='adropdown-input' >")
        };
        ...
        this.wrap("<div style='position: relative; display:inline-block; border:0px solid blue; width:100%'></div>");

        // create a textbox for input
        this.after(el.ddDivButon.append(el.ddTextbox));
        ...
        this.hide();
        el.ddDiv.append(el.ddDivContent);

        // append table after the new textbox
        el.ddDivContent.append(el.ddTable);

        // append table caption
        el.ddTable.append(el.ddTableCaption);

        // create table columns
        var header = "<thead style='display: block;overflow: auto;color: #fff;background: #EEE;'><tr>";
        header = header + "<th style='width:0px;display: none;'></th>";
        for (var i = 0; i <= cols - 1; i++) {
            header = header + "<th style='width:" + ((tblWidth / numberCol)) + "px'>" + settings.columns[i] + "</th>";
        }
        header = header + "</thead></tr>";
        el.ddTable.append(header);
        var body = "<tbody style='display: block;height: 200px;background: #FFF;overflow: auto;'>";
        el.ddTable.append(body);
        ...
        el.ddTextbox.click(function (event) {
        ...
        function select() {
            ...
            if (showAdd == true) {
                var textAdd = selected.find('td').eq(2).text();
                if (textAdd == "") {
                    textAdd = selected.find('td').eq(3).text();
                }

                if (textAdd == "") {
                    el.ddTextbox.val(selected.find('td').eq(1).text());
                } else {
                    el.ddTextbox.val(selected.find('td').eq(1).text() + "_" + textAdd);
                }
            } else {
                el.ddTextbox.val(selected.find('td').eq(1).text());
            }
        ...
        function showDropDown() {
            ...
            var cssTop = (el.ddTextbox.height() + 17) + "px 1px 0px 1px";
        ...
        function jsonParser(jsonData) {
            try {
                ...
                var i = 0, j = 0;
                var row = null, cell = null, strCall = "";
                if (jsonData != null) {
                    ...
                    for (i = 0; i < jsonData.length; i++) {
                        ...
                        for (var key in obj) {
                            ...
                            if (j <= cols) {
                                ...
                                cell = obj[key];
                                row = row + "<td style='width:" + tblWidth / numberCol + "px;word-wrap: break-word;word-break: break-all;'>" + cell + "</td>";
                                strCall += "_" + cell;
                            }
                            else {
                                continue;
                        ...
                        if (el.ddTextbox.val() != "") {
                            var s = el.ddTextbox.val();

                            if (strCall.indexOf(s) >= 0) {

                                el.ddTable.append("<tr class='selected'>" + row + "</tr>");
                            } else {
