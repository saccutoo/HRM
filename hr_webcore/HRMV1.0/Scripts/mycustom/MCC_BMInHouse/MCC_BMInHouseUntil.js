function BuildTable(appName, controllerName, tableUrl) {
    app.controller(controllerName,
        function ($scope, myService, $filter, $rootScope, $compile) {
            ...
            function ListType() {
                var data = {
                    url: "/Common/GetDataByGloballistnotTree?parentid=" + 1995
                }
                var list = myService.getDropdown(data);
                list.then(function (res) {
            ...
            //-----------------Bắt lỗi khi nhập BM_MCCID-----------------
            $scope.ChangeBM_MCC = function () {
                if ($scope.Data.BM_MCCId == null || $scope.Data.BM_MCCId == "") {
                    $scope.ErrorBM_MCCId = "Vui lòng nhập BM_MCC";
                    return
                }
                if (isNaN($scope.Data.BM_MCCId)) {
                    $scope.ErrorBM_MCCId = "BM_MCC phải là số";
                    return
                }
                var position = $scope.Data.BM_MCCId.toString().indexOf('.');
                if (position != -1) {
                    $scope.ErrorBM_MCCId = "Vui lòng nhập số nguyên";
                    return
                }
                if ($scope.Data.BM_MCCId.length>19) {
                    $scope.ErrorBM_MCCId = "Số kí tự tối là 19";
                    return
                }
                else {
                    $scope.ErrorBM_MCCId = "";                 
            ...
            //-----------------Bắt lỗi khi chọn nghề-----------------
            $scope.ChangeListType = function () {           
                if ($scope.Data.Type == null || $scope.Data.Type == "") {
                    $scope.ErrorType = "Vui lòng chọn công cụ";
                    return
                }
                else {
                    $scope.ErrorType = "";
                    if ($scope.Data.Type==1) {
                        $scope.Data.IsPartner = true;
                    }
                    else {
                        $scope.Data.IsPartner = false;
                    }
                }
            }

            //-----------------Bắt lỗi khi chọn tài khoản-----------------
            $scope.ChangeAccountType = function () {
            
                if ($scope.Data.AccountType == null || $scope.Data.AccountType == "") {
                    $scope.ErrorAccountType = "Vui lòng chọn loại tài khoản";
                    return
                }
                else {
            ...
            $scope.editClick = function (contentItem) {
                ...
                $scope.Data.BM_MCCId = contentItem.BM_MCCId;
                $scope.Data.AccountType = contentItem.AccountType;
                $scope.Data.Type = contentItem.Type.toString();
                $scope.Data.IsPartner = contentItem.IsPartner;
                ShowPopup($,
                   "#SaveMCC_BMlnHouse",
                   $scope.tableInfo.PopupWidth,
                   $scope.tableInfo.PopupHeight);
