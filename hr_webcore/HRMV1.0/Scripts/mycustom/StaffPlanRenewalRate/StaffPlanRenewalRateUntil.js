    ...
    app.controller(controllerName,
        function ($scope, myService, $filter, $rootScope, $compile, $http) {
            ...
            $scope.addClick = function () {
                ...
                $scope.StaffPlanRenewalRateModel = {};
                var today = new Date();
                var yyyy = today.getFullYear();
                $scope.StaffPlanRenewalRateModel.Year = yyyy;
                ShowPopup($,
                    "#StaffPlanRenewalRateSave",
            ...
            $scope.SaveEditClick = function (tblDatas) {
                ...
                if ($scope.ValidationButtonUpdate(lstTemp)) {
                    var SaveAction = myService.UpdateData("/StaffPlanRenewalRate/StaffPlanRenewalRate_Save", lstTemp);
                    if (SaveAction != null) {
                        SaveAction.then(function (res) {
                            if (res.data.result.IsSuccess == true) {
                                AppendToToastr(true, notification, successfulUpdate, 500, 5000);
                                $scope.StaffPlanRenewalRateData.reload();
                                $scope.Data = [];
                                $scope.CloseForm();
                            }
                            else {
            ...
            $scope.Save = function (form) {
                ...
                if (!form.$error.required) {
                    var lstTemp = [];
                    lstTemp.push($scope.StaffPlanRenewalRateModel);
                    var SaveAction = $http({
                        method: "POST",
                        url: "/StaffPlanRenewalRate/StaffPlanRenewalRate_Save",
                        data: {
                            data: lstTemp
                        },
                        dataType: "json"
                    ...
                    SaveAction.then(function (res) {
                        if (res.data.result.IsSuccess == true) {
                            $scope.editFromsubmit = false;
                            if ($scope.StaffPlanRenewalRateModel.AutoID != 0 || $scope.StaffPlanRenewalRateModel.AutoID != null) {
                                AppendToToastr(true, notification, successfulUpdate, 500, 5000);
                            }
                            else {
