    ...
    app.controller(controllerName,
        function ($scope, myService, $filter, $rootScope) {
            ...
            $scope.choseClick = function (contentItem) {
                ...
                var edit = myService.getDataById(contentItem.RoleID, 11, '/Sec_Role_Organizationunit/GetOrganizationUnitByIdRole');
                ...
                edit.then(function (emp) {
                    ...
                    for (var i = 0; i < emp.data.result.length; i++) {
                        $rootScope.temp.push(emp.data.result[i].OrganizationUnitID);
                    }
                ...
                if ($rootScope.idrolre != null && $rootScope.idrolre1 != null) {
                    for (var i = 0; i < $rootScope.idrolre.length; i++) {
                        for (var j = 0; j < $rootScope.idrolre1.length; j++) {
                            ...
                            myService.UpdateData('/Sec_Role_Organizationunit/UpdateSec_Role_Organizationunit', $scope.editData);
                        }
                    }
                    AppendToToastr(true, notification, setRoleSuccessful, 500, 5000);
                }
                else {
                    AppendToToastr(false, notification, youMustChooseRoleAnDept, 500, 5000);
                }
            }
            //Load dữ liệu
            ...
            var getItem = function (index) {
                var item = {
                    id: sampleData[index].OrganizationUnitID,
