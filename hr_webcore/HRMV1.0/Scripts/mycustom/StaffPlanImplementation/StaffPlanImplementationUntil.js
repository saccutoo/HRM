        ...
        function ($scope, myService, $filter, $rootScope, $compile, $http) {
            ...
            $scope.ChangeYear = function (employee) {
                if (employee.Data.Year == null || employee.Data.Year == "") {
                    ...
                    employee.ErrorYear = errorMoneyIsNumber;
                    return
                }
                else {
                    employee.ErrorYear = "";
            ...
            $scope.SaveEditClick = function (tblDatas) {
                ...
                if ($scope.Data.length > 0 && $scope.Data!=null) {
                    var SaveAction = myService.UpdateData("/StaffPlanImplementation/StaffPlanImplementation_Save", $scope.Data);
                    SaveAction.then(function (res) {
                        if (res.data.result.IsSuccess == true) {
                            AppendToToastr(true, notification, successfulUpdate, 500, 5000);
                            $scope.StaffPlanImplementationData.reload();
                            $scope.Data = [];
                            $scope.CloseForm();
                        }
                        else {
                            AppendToToastr(false, notification, successfulUpdate, 500, 5000);
                            $scope.StaffPlanImplementationData.reload();
                            $scope.Data = [];
                            $scope.CloseForm();
            ...
            $scope.downLoadFileClick = function () {
                window.location = "/StaffPlanImplementation/DownloadTemplate";
            }
            ...
            $scope.upload = function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: "/StaffPlanImplementation/ImportExcel",
                    data: $formData,
                    processData: false,
                    contentType: false,
                    beforeSend: function () {
                    ...
                    success: function (data) {
                        ...
                        if (data1.SM.IsSuccess == false) {
                            ...
                            if (data1 != null && data1.result.length > 0) {
                                window.location = "/StaffPlanImplementation/ExportStaffPlanImplementationDataError";
                            }
                            else {
            ...
            $scope.ChangeAddYear = function () {
                if ($scope.Data.Year == null || $scope.Data.Year == "") {
                    $scope.ErrorYear = "Vui lòng chọn năm.";
                    return;
                }
                else {
                    $scope.ErrorYear = "";
            ...
            $scope.Save = function () {
                ...
                if ($scope.Data.Year == null || $scope.Data.Year == "") {
                    ...
                    var SaveAction = myService.UpdateData("/StaffPlanImplementation/StaffPlanImplementation_Save", $scope.ListData);
                    SaveAction.then(function (res) {
                        if (res.data.result.IsSuccess == true) {
