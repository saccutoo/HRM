    ...
    app.controller(controllerName,
        function ($scope, myService, $filter, $rootScope, $window, $http, $compile) {
            ...
            $scope.getColumns = function (DoNotLoad) {
                if (tableUrl == "/Salary/TableServerSideGetData") {
            ...
            //$scope.LatchesAll = function (id) {
            //    BoostrapDialogConfirm(notification,
            //        "Bạn có muốn chốt toàn bộ phiếu lương ?",
            //        BootstrapDialog.TYPE_WARNING,
            //        $scope.LatchesWorkDay,
            //        id);
            //}
            ...
            $scope.AllStaff = false;
            $scope.LatchesWorkDay = function (Period, type) {
                var url = '';
                if (type == 1) {
                    url = '/Salary/LatchesWorkDay';
                }
                else { url = '/Salary/RemoveLatchesWorkDay'; }
                if ($scope.list.employees != null) {
                    $scope.list.StaffID = $scope.list.employees.map(a => a.StaffID);
                    //$scope.list.OrganizationUnitID = $scope.list.employees.map(a => a.OrganizationUnitID);
                }
                var dt = Loading();
                $scope.from = $scope.FromMonth.split("/");
                if ($scope.list.StaffID.length > 0 || $scope.AllStaff == true) {//đã chọn chốt
                    ...
                    var list = myService.LatchesWorkDay(data);
                    list.then(function (res) {
                        $scope.refesh();
                        $scope.GetListData();
                        dt.finish();
            ...
            $scope.CreatePayslip = function () {
                ...
                if ($scope.list.StaffID.length > 0 || $scope.AllStaff == true) {//đã chọn chốt
                    $scope.from = $("#cb1").prop("checked") == false ? $scope.list.employees[0].MonthYear.split("/") : $scope.FromMonth.split("/");
                    var ischeckAll = false;
                    if ($scope.AllStaff == true) {
                        ischeckAll = true;
                    }
            ...
            $scope.SendMail = function (id) {
                ...
                if ($scope.list.StaffID.length > 0 || $scope.AllStaff == true) {//đã chọn chốt
                    ...
                    list.then(function (res) {
                        ...
                        $scope.refesh();
                        dt.finish();
                        AppendToToastr(result, notification, res.data.sendMessage + res.data.listNameStaff, 500, 5000);
            ...
            $scope.UpdateBonus = function (type) {
                if ($scope.list.employees != null) {
                    $scope.list.StaffID = $scope.list.employees.map(a => a.StaffID);
                    $scope.list.WPID = $scope.list.employees.map(a => a.WPID);
                ...
                if ($scope.list.StaffID.length > 0 || $scope.AllStaff == true) {//đã chọn chốt
                    ...
                    var url = "/Salary/UpdateBonus";
                    var data = {
                        listID: $scope.list.StaffID,
                        listWPID: $scope.list.WPID,
                        month: $scope.from[0].replace("T", ""),
                        year: $scope.from[1],
                        policyBonusIDRun: type,
                        isCheckAll: ischeckAll
                    };
                    var updateAction = myService.UpdateData(url, data);
                    updateAction.then(function (res) {
                        $scope.refesh();
                        $scope.GetListData();
                        dt.finish();
                    ...
                    dt.finish();
                    AppendToToastr(false, notification, selectCheckBoxBonus, 500, 3000);
                }
