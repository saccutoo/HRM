function BuildTable(appName, controllerName, tableUrl) {
    app.controller(controllerName,
        function ($scope, myService, $filter, $rootScope, $compile) {
            ...
            $scope.showReportLead = true;
            $scope.FromDateToDate1 = true; //cho phep build table hien thi tu ngay nao den ngay nao
            $scope.ReportOrganizationUnit = true; //cho phep build table hien thi tim kiem theo phong ban
            ...
            $scope.getTableInfo = function () {
                ...
                list.then(function (res) {
                    $rootScope.getListAllOrganizationUnit = res.data.result;
                }, function (res) {
                    $scope.msg = "Error";
                });
            ...
            $scope.GetListData = function () {
                var x = FormatDate($scope.FromDate);
                var x1 = x.split("/").reverse().join("-");
                var y = FormatDate($scope.ToDate);
                var y1 = y.split("/").reverse().join("-");
                $scope.startdate = x1 + " 00:00:00";
                $scope.enddate = y1 + " 00:00:00";
                ShowLoader();
                var data = {
            ...
            $scope.Save = function () {
                ...
                if (seconds < 0) {
                    $scope.Mesenger = "Vui lòng chọn ngày kết thúc không nhỏ hơn ngày bắt đầu";
                    temp = 1;
                }
                else if ($scope.editData.Type == null) {
                    $scope.Mesenger = "Vui lòng chọn loại";
                    temp = 1;
                }
